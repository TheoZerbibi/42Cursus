...
/*   Updated: 2020/09/19 07:46:42 by thzeribi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
**				This function threat `c` or `%` flag, is used to prepared and
**				show display.
**
**			@param t_tab *tab
**			@return tab
...
t_tab	*print_c(t_tab *tab)
{
	...
	if (tab->combin[1] == '0' && tab->combin[0] != '-' && tab->width_is_neg == 0)
		display(tab, '0', tab->width - 1, TRUE);
	else if (tab->width_is_neg == 0 && tab->combin[0] != '-')
		display(tab, ' ', tab->width - 1, TRUE);
